...
import { Plus, Edit2, Trash2, Target, Search, X, CheckCircle, Save, Upload } from 'lucide-react';
...
export default function AdminPrelimPYQsPage() {
  ...
  const [form, setForm] = useState({
    examId: '', subjectId: '', subCategory: '', year: new Date().getFullYear(), difficulty: 'medium',
  ...
  const bulkColumns = [
    ...
    { key: 'correctans', label: 'Correct Answer', required: true },
  ...
  const resetForm = () => {
    setForm({ examId: '', subjectId: '', subCategory: '', year: new Date().getFullYear(), difficulty: 'medium', text: '', options: ['', '', '', ''], correctAns: '', explanation: '' });
    setEditId(null);
    setShowForm(false);
  };
  ...
  const handleEdit = (q: any) => {
    setForm({
      examId: q.examId || '', subjectId: q.subjectId || '', subCategory: q.subCategory || '', year: q.year || new Date().getFullYear(),
      difficulty: q.difficulty || 'medium', text: q.text || '',
      options: Array.isArray(q.options) ? q.options : ['', '', '', ''],
      correctAns: q.correctAns || '', explanation: q.explanation || '',
    });
    setEditId(q.id);
    setShowForm(true);
  };
  ...
  const filtered = questions.filter(q => {
    ...
    const mExam = !filterExam || q.examId === filterExam;
    const mSub = !filterSubCategory || q.subCategory === filterSubCategory;
    return mSearch && mYear && mExam && mSub;
  });
  ...
  return (
    <div className="animate-fade-in">
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-2xl font-bold text-surface-900">Prelim PYQs</h1>
          <p className="text-sm text-surface-500 mt-1">Manage objective / MCQ-style previous year questions</p>
        </div>
        <div className="flex gap-2">
          ...
          <Button onClick={() => { resetForm(); setShowForm(true); setShowBulkImport(false); }}>
            <Plus className="h-4 w-4 mr-1" /> Add Prelim PYQ
          </Button>
        </div>
      </div>
        ...
        <Card className="mb-6 border-brand-200">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              {editId ? 'Edit Prelim Question' : 'Add New Prelim Question'}
            </CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="mb-4">
              <label className="text-sm font-medium text-surface-700 mb-1.5 block">Explanation</label>
              <textarea
      ...
      {showBulkImport && (
        <BulkImport
          columns={bulkColumns}
          templateFilename="prelim-pyq-template.csv"
          templateRows={[
            ['OSSC', 'History', 'Ancient India', '2025', 'medium', 'Which dynasty ruled?', 'Maurya', 'Gupta', 'Chola', 'Pandya', 'Maurya', 'The Maurya dynasty ruled from 322 BCE.'],
            ['OPSC', 'Polity', 'Constitution', '2024', 'easy', 'Who is the head of state?', 'President', 'PM', 'CJI', 'Speaker', 'President', 'The President is the constitutional head.'],
          ]}
              ...
              <tbody>
                {filtered.map((q, i) => (
                  <tr key={q.id} className="border-b border-surface-100 hover:bg-surface-50 transition-colors">
                    ...
                    <td className="px-4 py-3 text-surface-600">{q.examId || '-'}</td>
      ...
      {filtered.length === 0 && (
        <div className="text-center py-20">
          <Target className="h-10 w-10 text-surface-300 mx-auto mb-3" />
          <p className="text-surface-500">No prelim questions found.</p>
