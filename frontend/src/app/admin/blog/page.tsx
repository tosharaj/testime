'use client';
import { useEffect, useState } from 'react';
import { Card, CardContent } from '@/components/ui/Card';
import Badge from '@/components/ui/Badge';
import Button from '@/components/ui/Button';
import { Plus, Edit2, Trash2, Eye, Newspaper } from 'lucide-react';
import { formatDate } from '@/lib/utils';

export default function AdminBlogPage() {
  const [posts, setPosts] = useState<any[]>([]);

  useEffect(() => {
    fetch('http://localhost:4000/api/blog', {
      headers: { Authorization: `Bearer ${localStorage.getItem('token')}` }
    }).then(r => r.json()).then(d => setPosts(d.data || [])).catch(console.error);
  }, []);

  return (
    <div>
      <div className="flex items-center justify-between mb-6">
        <h1 className="text-2xl font-bold text-surface-900">Blog Posts</h1>
        <Button><Plus className="h-4 w-4 mr-1" /> New Post</Button>
      </div>

      <Card>
        <CardContent className="p-0">
          <div className="overflow-x-auto">
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-surface-200 bg-surface-50">
                  <th className="text-left px-4 py-3 font-medium text-surface-700">Title</th>
                  <th className="text-left px-4 py-3 font-medium text-surface-700">Status</th>
                  <th className="text-left px-4 py-3 font-medium text-surface-700">Published</th>
                  <th className="text-right px-4 py-3 font-medium text-surface-700">Actions</th>
                </tr>
              </thead>
              <tbody>
                {posts.map((post) => (
                  <tr key={post.id} className="border-b border-surface-100 hover:bg-surface-50 transition-colors">
                    <td className="px-4 py-3 font-medium text-surface-900">{post.title}</td>
                    <td className="px-4 py-3">
                      <Badge variant={post.isPublished ? 'success' : 'default'}>{post.isPublished ? 'Published' : 'Draft'}</Badge>
                    </td>
                    <td className="px-4 py-3 text-surface-500">{post.publishedAt ? formatDate(post.publishedAt) : '-'}</td>
                    <td className="px-4 py-3 text-right">
                      <button className="p-1.5 text-surface-400 hover:text-brand-600 transition-colors"><Eye className="h-4 w-4" /></button>
                      <button className="p-1.5 text-surface-400 hover:text-amber-600 transition-colors"><Edit2 className="h-4 w-4" /></button>
                      <button className="p-1.5 text-surface-400 hover:text-red-600 transition-colors"><Trash2 className="h-4 w-4" /></button>
                    </td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
        </CardContent>
      </Card>

      {posts.length === 0 && (
        <div className="text-center py-16">
          <Newspaper className="h-10 w-10 text-surface-300 mx-auto mb-3" />
          <p className="text-surface-500">No blog posts yet.</p>
        </div>
      )}
    </div>
  );
}
